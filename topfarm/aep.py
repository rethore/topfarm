...
class AEP(object):
    """ Calculate the AEP of a given wind farm
    Parameters
    ----------
    ...
    def __init__(self,
        ...
        self.speed_up_il = np.zeros([self.num_turbines, self.num_wd_bins])
        self.turning_il = np.zeros([self.num_turbines, self.num_wd_bins])
        self.inclination_il = np.zeros([self.num_turbines, self.num_wd_bins])
        self.Weibull_A_il = np.zeros([self.num_turbines, self.num_wd_bins])
        self.Weibull_k_il = np.zeros([self.num_turbines, self.num_wd_bins])
        self.frequency_il = np.zeros([self.num_turbines, self.num_wd_bins])
        self.turbulence_il = np.zeros([self.num_turbines, self.num_wd_bins])
        self.wind_shear_il = np.zeros([self.num_turbines, self.num_wd_bins])
        self.rho_il = np.zeros([self.num_turbines, self.num_wd_bins])

        # Initialize the wt_postions from self.wf_design
    ...
    def set_wt_positions(self, wt_positions=None):
        """Set new wt positions
        """
        if wt_positions == None:
            # We get default values from wt_design
            self.wt_positions = self.wf_design[:, (0, 1, 4)]
        else:
            selt.wt_positions = wt_positions
    ...
    def cal_wt_conditions(self):
        ...
        for l_wd in range(self.num_wd_bins):
            ...
            conditions = self.site_conditions.get_site_conditions(
                self.wt_positions, wd)

            self.Weibull_A_il[:, l_wd] = conditions['A'].values
            self.Weibull_k_il[:, l_wd] = conditions['k'].values
            self.frequency_il[:, l_wd] = conditions['freq_per_degree'].values
            self.turbulence_il[:, l_wd] = conditions['tke_amb'].values

            if self.terrain_type == 'complex':
                self.speed_up_il[:, l_wd] = conditions['spd_up'].values
